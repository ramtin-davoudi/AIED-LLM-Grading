...
TRAIN_SPLIT = "train"
TEST_SPLIT = "test_ua"
MODEL = "gpt-4o"   # change to gpt-4o for the other run

MAX_ROWS = 540     
POLL_SECONDS = 15
RANDOM_SEED = 42

# 15-shot = 3 examples per each of 5 labels
EXAMPLES_PER_LABEL = 3

OUT_DIR = Path("scientsbank_batch_run")
OUT_DIR.mkdir(exist_ok=True)

FEW_SHOT_PATH = OUT_DIR / "few_shot_examples_15shot.json"
BATCH_INPUT_PATH = OUT_DIR / f"batch_input_{MODEL}_fifteen_shot.jsonl"
RAW_RESULTS_PATH = OUT_DIR / f"batch_output_raw_{MODEL}_fifteen_shot.jsonl"
RAW_ERRORS_PATH = OUT_DIR / f"batch_errors_raw_{MODEL}_fifteen_shot.jsonl"
OUTPUT_CSV_PATH = OUT_DIR / f"scientsbank_{TEST_SPLIT}_{MODEL}_with_llm_labels_fifteen_shot.csv"
...
def select_and_save_few_shot_examples(
    train_df,
    few_shot_path,
    examples_per_label=3,
    random_seed=42,
    ...
    """
    Select a fixed number of examples per label from the training set and save them.
    ...
    # optional: sort by label for consistent prompt ordering
    examples = sorted(examples, key=lambda x: x["label"])
...
def format_few_shot_examples(examples):
    """
    Format all few-shot examples into the prompt.
    """
    blocks = []
...
Below are labeled examples of the grading categories.
...
# =========================
# Step 2: Get fixed 15-shot examples
# =========================
...
print(f"\nTotal few-shot examples: {len(few_shot_examples)}")
